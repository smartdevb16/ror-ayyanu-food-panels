class Business::EnterprisesController < ApplicationController
  ...
  def add_new_menu_item
    ...
    if restaurant
      if !item
        status = is_new_menu_item(params[:item_name].strip, params[:price_per_item].strip, params[:image], params[:item_description].strip, params[:menu_category_id].strip, params[:is_available].strip, params[:item_name_ar].strip, params[:item_description_ar].strip, params[:calorie].strip, false, params[:addon_category_id], params[:dish_date], params[:far_menu])
  ...
  def menu_new_addon_item
    ...
    if restaurant
      ...
      if category
        addon = add_new_addon_item(category, params[:item_name].strip, params[:price_per_item].strip, params[:item_name_ar].strip, false, params[:available])
        @admin = get_admin_user
        restaurant = restaurant
        title = restaurant.title
  ...
  def update_menu_addon_item
    ...
    if restaurant
      ...
      if addon_item
        addonItem = update_addon(addon_item, params[:item_name].strip, params[:price_per_item].strip, params[:addon_category_id].strip, params[:item_name_ar].strip, false, params[:available])
        @admin = get_admin_user
        restaurant = restaurant
        title = restaurant.title
      ...
      addon_item = get_addon_item(params[:addon_item_id])
      if addon_item
        addonItem = update_addon(addon_item, params[:item_name].strip, params[:price_per_item].strip, params[:addon_category_id].strip, params[:item_name_ar], false, params[:available])
        @admin = get_admin_user
        restaurant = @user.manager_branches.first.restaurant
        title = restaurant.title
        msg = "#{title} restaurant has update addon item"
