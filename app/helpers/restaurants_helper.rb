module RestaurantsHelper
  ...
  def is_new_menu_item(item_name, price_per_item, item_image, item_description, menu_category_id, is_available, item_name_ar, item_description_ar, calorie, approve, addon_category_id, item_date, far_menu, include_in_pos, include_in_app, preparation_time = 15, recipe_ids, station_ids)
    image = item_image.present? ? upload_multipart_image(item_image, "menu_item") : nil
    MenuItem.newMenuItem(item_name, price_per_item, image, item_description, menu_category_id, is_available, item_name_ar, item_description_ar, calorie, approve, addon_category_id, item_date, far_menu, include_in_pos, include_in_app, preparation_time, recipe_ids, station_ids)
  end

  def is_update_menu_item(item, item_name, price_per_item, new_item_image, item_description, menu_category_id, is_available, item_name_ar, item_description_ar, approve, calorie, item_date, addon_category_id, far_menu, include_in_pos=nil, include_in_app=nil, menu_item_ids, recipe_ids, station_ids)
  ...
  def get_request_restaurant(restaurant_id)
    NewRestaurant.find_new_restaurant(restaurant_id)
  end

  def get_restaurant_doc(doc_id)
    NewRestaurantImage.find_by(id: doc_id)
  end
    ...
    if data
      ...
      if user[:code] == 200
        ...
        if req_restaurant.restaurant.present?
          ...
          restaurant = Restaurant.create_restaurant(req_restaurant, user)
        end
  ...
  def add_new_addon_item(category, item_name, price_per_item, item_name_ar, approve, available, include_in_pos, include_in_app, preparation_time = 15)
    ItemAddon.create_new_addon_item(category, item_name, price_per_item, item_name_ar, approve, available, include_in_pos, include_in_app, preparation_time)
  end
  ...
  def update_addon(addon_item, item_name, price_per_item, addon_category_id, item_name_ar, approve, available, include_in_pos, include_in_app, preparation_time = 15)
    addon_item.update(addon_title: item_name, addon_price: price_per_item, item_addon_category_id: addon_category_id, addon_title_ar: item_name_ar, approve: approve, is_rejected: false, available: available, include_in_pos: include_in_pos, include_in_app: include_in_app, preparation_time: preparation_time)
  end
